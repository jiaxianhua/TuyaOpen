...
static void open_item_for_view(void)
{
    ...
    if (is_epub) {
        char epub_dir[256];
        if (epub_extract_all(full_path, PROGRESS_DIR, epub_dir, sizeof(epub_dir)) == 0) {
            TUYA_DIR dir = NULL;
            if (tkl_dir_open(epub_dir, &dir) == OPRT_OK) {
                tkl_dir_close(dir);
                strncpy(sg_app_ctx.current_path, epub_dir, sizeof(sg_app_ctx.current_path) - 1);
                sg_app_ctx.current_path[sizeof(sg_app_ctx.current_path) - 1] = 0;
                sg_app_ctx.state                                             = STATE_FILE_LIST;
                sg_app_ctx.current_page                                      = 0;
                sg_app_ctx.selected_index                                    = 0;
    ...
    sg_app_ctx.viewing_epub                                      = FALSE;
    sg_app_ctx.epub_root[0]                                      = 0;
    ...
    if (sg_app_ctx.view_kind == VIEW_TEXT && is_epub_cache_dir(sg_app_ctx.current_path)) {
        sg_app_ctx.viewing_epub = TRUE;
        strncpy(sg_app_ctx.epub_root, sg_app_ctx.current_path, sizeof(sg_app_ctx.epub_root) - 1);
        sg_app_ctx.epub_root[sizeof(sg_app_ctx.epub_root) - 1] = 0;
    }
    sg_app_ctx.viewing_size   = sg_app_ctx.viewing_data[0] ? tkl_fgetsize(sg_app_ctx.viewing_data) : 0;
    INT64_T bom               = 0;
    sg_app_ctx.viewing_enc    = (sg_app_ctx.view_kind == VIEW_TEXT && sg_app_ctx.viewing_data[0])
                                    ? detect_file_encoding(sg_app_ctx.viewing_data, &bom)
                                    : VIEW_ENC_GBK;
...
static BOOL_T is_epub_cache_dir(const char *path)
{
    if (!path)
        return FALSE;
    return (strstr(path, PROGRESS_DIR "/epub_") != NULL) ? TRUE : FALSE;
}

static INT64_T epub_page_process(const char *path, INT64_T start_off, VIEW_ENC_E enc, int max_w, int max_h,
                                 const char *epub_root, BOOL_T draw, int draw_y)
{
    if (!path || !epub_root)
        return start_off;
    if (max_h <= 0)
    ...
    size_t pos    = 0;
    int    y      = draw_y;
    int    bottom = draw_y + max_h;
    while (pos < (size_t)rd) {
        if (y + TEXT_LINE_HEIGHT > bottom)
            break;

        if (enc == VIEW_ENC_UTF16LE || enc == VIEW_ENC_UTF16BE) {
            size_t step = advance_one_line_in_buf_utf16(win + pos, (size_t)rd - pos, (enc == VIEW_ENC_UTF16LE), max_w);
            if (step == 0)
                break;
            size_t line_len = step;
            size_t trim     = line_len;
            while (trim >= 2) {
                uint16_t u = (enc == VIEW_ENC_UTF16LE)
                                 ? (uint16_t)win[pos + trim - 2] | ((uint16_t)win[pos + trim - 1] << 8)
                                 : (uint16_t)win[pos + trim - 1] | ((uint16_t)win[pos + trim - 2] << 8);
            ...
            if (draw && trim > 0) {
                uint8_t utf8_buf[512];
                int utf8_len = utf16_to_utf8_buf(win + pos, trim, (enc == VIEW_ENC_UTF16LE) ? TRUE : FALSE, utf8_buf,
                                                 sizeof(utf8_buf) - 1);
                if (utf8_len < 0)
                    utf8_len = 0;
                utf8_buf[utf8_len] = 0;
                char gbk_buf[1024];
                int  gbk_len = utf8_to_gbk_buf(utf8_buf, (size_t)utf8_len, (uint8_t *)gbk_buf, sizeof(gbk_buf) - 1);
                if (gbk_len < 0)
                    gbk_len = 0;
                gbk_buf[gbk_len] = 0;
                Paint_DrawText_CN_HZK24_Adaptive(TEXT_MARGIN_X, y, max_w, TEXT_LINE_HEIGHT, gbk_buf, BLACK, WHITE);
            }
            y += TEXT_LINE_HEIGHT;
            pos += step;
            continue;
        ...
        if (pos + 5 < (size_t)rd && win[pos] == '[' && memcmp(win + pos, "[IMG:", 5) == 0) {
            size_t j = pos + 5;
            while (j < (size_t)rd && win[j] != ']' && win[j] != '\n' && win[j] != '\r' && (j - pos) < 240) {
                j++;
            }
            if (j < (size_t)rd && win[j] == ']') {
                char   rel[256];
                size_t rel_len = j - (pos + 5);
                if (rel_len >= sizeof(rel))
                    rel_len = sizeof(rel) - 1;
                memcpy(rel, win + pos + 5, rel_len);
                rel[rel_len] = 0;

                size_t after = j + 1;
                while (after < (size_t)rd && win[after] != '\n' && win[after] != '\r')
                    after++;
                if (after < (size_t)rd && win[after] == '\r') {
                    after++;
                    if (after < (size_t)rd && win[after] == '\n')
                        after++;
                } else if (after < (size_t)rd && win[after] == '\n') {
                    after++;
                }

                int remain = bottom - y;
                if (remain < TEXT_LINE_HEIGHT)
                    break;
                int img_h = remain;
                if (img_h > 200)
                    img_h = 200;
                if (draw && rel[0]) {
                    char img_path[512];
                    snprintf(img_path, sizeof(img_path), "%s/%s", epub_root, rel);
                    (void)display_image_1bit(img_path, TEXT_MARGIN_X, y, max_w, img_h);
                }
                y += img_h + (TEXT_LINE_HEIGHT / 2);
                pos = after;
                continue;
            }
        }

        size_t step = advance_one_line_in_buf_8bit(win + pos, (size_t)rd - pos, enc, max_w);
        if (step == 0)
            break;
        size_t line_len = step;
        while (line_len > 0 && (win[pos + line_len - 1] == '\n' || win[pos + line_len - 1] == '\r')) {
            line_len--;
        }
        ...
        if (draw && line_len > 0) {
            ...
            memcpy(line_buf, win + pos, line_len);
            line_buf[line_len] = 0;
            if (enc == VIEW_ENC_UTF8) {
                char gbk_buf[1024];
                int  gbk_len = utf8_to_gbk_buf(line_buf, line_len, (uint8_t *)gbk_buf, sizeof(gbk_buf) - 1);
                if (gbk_len < 0)
                    gbk_len = 0;
                gbk_buf[gbk_len] = 0;
                Paint_DrawText_CN_HZK24_Adaptive(TEXT_MARGIN_X, y, max_w, TEXT_LINE_HEIGHT, gbk_buf, BLACK, WHITE);
            } else {
                Paint_DrawText_CN_HZK24_Adaptive(TEXT_MARGIN_X, y, max_w, TEXT_LINE_HEIGHT, (char *)line_buf, BLACK,
                                                 WHITE);
            }
        }
        y += TEXT_LINE_HEIGHT;
...
static void refresh_ui(void)
{
    ...
    if (sg_app_ctx.state == STATE_FILE_LIST) {
        ...
        if (sg_app_ctx.view_kind == VIEW_TEXT) {
            INT64_T end_off = 0;
            if (sg_app_ctx.viewing_epub) {
                end_off = epub_page_process(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, sg_app_ctx.viewing_enc,
                                            max_w, content_h, sg_app_ctx.epub_root, FALSE, content_y);
            } else {
                end_off = advance_lines_in_file(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, lines_per_page,
        ...
        if (sg_app_ctx.view_kind == VIEW_IMAGE) {
            ...
            if (ext_eq(file_ext(sg_app_ctx.viewing_file), "pdf")) {
                ...
                if (sg_app_ctx.viewing_epub) {
                    (void)epub_page_process(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, sg_app_ctx.viewing_enc,
                                            max_w, content_h, sg_app_ctx.epub_root, TRUE, content_y);
                } else {
                    INT64_T end_off = advance_lines_in_file(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset,
                                ...
                                if (sg_app_ctx.viewing_enc == VIEW_ENC_UTF8) {
                                    ...
                                    if (utf8) {
                                        int utf8_len = utf16_to_utf8_buf(
                                            (uint8_t *)raw, rd,
                                            (sg_app_ctx.viewing_enc == VIEW_ENC_UTF16LE) ? TRUE : FALSE, utf8,
                                            utf8_cap - 1);
                                        if (utf8_len < 0)
                                            utf8_len = 0;
                                        utf8[utf8_len] = 0;
                                        ...
                                        if (gbk) {
                                            ...
                                            if (out_len < 0)
                                                out_len = 0;
                                            gbk[out_len] = 0;
                                            Paint_DrawText_CN_HZK24_Adaptive(TEXT_MARGIN_X, content_y, max_w, avail_h,
                                                                             gbk, BLACK, WHITE);
                                            tal_free(gbk);
                                        } else {
                                            Paint_DrawString_EN(10, 50, "Memory Error", &Font24, BLACK, WHITE);
                                    ...
                                    Paint_DrawText_CN_HZK24_Adaptive(TEXT_MARGIN_X, content_y, max_w, avail_h, raw,
                                                                     BLACK, WHITE);
                                }
                                tal_free(raw);
                            } else {
        ...
        for (int i = 0; i < page_count; i++) {
            ...
            const char *prefix    = fi.downloaded ? "D " : "";
            int         prefix_px = fi.downloaded ? (int)strlen(prefix) * (int)Font24.Width : 0;
            int         name_x    = 10 + prefix_px;
            ...
            if (ascii) {
                char clipped[140];
                int  name_max_px = size_x - 10 - Font24.Width - prefix_px;
                if (name_max_px < 24)
                    name_max_px = 24;
                clip_name_to_px(clipped, sizeof(clipped), name_utf8, name_max_px);
                if (prefix_px) {
                    Paint_DrawString_EN(10, y_pos, prefix, &Font24, fg, bg);
                }
                Paint_DrawString_EN((UWORD)name_x, y_pos, clipped, &Font24, fg, bg);
            } else {
                uint8_t gbk[256];
                ...
                Paint_DrawString_CN_HZK24((UWORD)name_x, y_pos, clipped, fg, bg);
            }
        ...
        Paint_DrawString_EN(10, Paint.Height - 28, "MID:Detail MID(hold):DL RST:Back RST(hold):SD", &Font24, BLACK,
                            WHITE);
        ...
        Paint_DrawLine(10, 35, Paint.Width - 10, 35, BLACK, DOT_PIXEL_1X1, LINE_STYLE_SOLID);
        char name_line[48];
        snprintf(name_line, sizeof(name_line), "Name:");
        Paint_DrawString_EN(10, 45, name_line, &Font24, BLACK, WHITE);
        Paint_DrawString_EN(10, 73, fi.name, &Font24, BLACK, WHITE);
        ...
        Paint_DrawString_EN(10, 101, size_line, &Font24, BLACK, WHITE);

        Paint_DrawString_EN(10, 129, "Path:", &Font24, BLACK, WHITE);
        Paint_DrawString_EN(10, 157, fi.path, &Font24, BLACK, WHITE);

        Paint_DrawString_EN(10, Paint.Height - 28, "MID:Back MID(hold):DL RST(hold):SD", &Font24, BLACK, WHITE);
    ...
    if (sg_app_ctx.state == STATE_FILE_LIST) {
        ...
        if (strcmp(name, "RST") == 0) {
            ...
            if (strcmp(name, "DOWN") == 0) {
                ...
                INT64_T next = 0;
                if (sg_app_ctx.viewing_epub) {
                    next = epub_page_process(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, sg_app_ctx.viewing_enc,
                                             max_w, TEXT_LINE_HEIGHT, sg_app_ctx.epub_root, FALSE, 0);
                } else {
                    next = advance_lines_in_file(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, 1,
                                                 sg_app_ctx.viewing_enc, max_w);
                }
                ...
                if (sg_app_ctx.viewing_epub) {
                    next = epub_page_process(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, sg_app_ctx.viewing_enc,
                                             max_w, content_h, sg_app_ctx.epub_root, FALSE, 0);
                } else {
                    next = advance_lines_in_file(sg_app_ctx.viewing_data, sg_app_ctx.viewing_offset, lines_per_page,
                                                 sg_app_ctx.viewing_enc, max_w);
                }
                if (next > sg_app_ctx.viewing_offset) {
                    sg_app_ctx.viewing_offset = next;
        ...
        if (strcmp(name, "RST") == 0) {
            ...
            if (strcmp(name, "UP") == 0) {
                ...
                if (event == TDL_BUTTON_LONG_PRESS_START) {
                    bdndk_request_download(idx, SDCARD_MOUNT_PATH "/TuyaT5AI");
                    sg_app_ctx.state = STATE_BD_MSG;
                    changed          = TRUE;
                } else {
                    bdndk_select_detail(idx);
                    sg_app_ctx.state = STATE_BD_DETAIL;
                    changed          = TRUE;
                }
            }
        } else if (sg_app_ctx.state == STATE_BD_DETAIL) {
            if (strcmp(name, "MID") == 0 && event == TDL_BUTTON_LONG_PRESS_START) {
                BDNDK_FILE_T fi;
                memset(&fi, 0, sizeof(fi));
                if (bdndk_detail_get(&fi)) {
                    int idx = -1;
                    for (int i = 0; i < bdndk_list_count(); i++) {
                        BDNDK_FILE_T tmp;
                        if (bdndk_list_get(i, &tmp) && strcmp(tmp.fsid, fi.fsid) == 0) {
                            idx = i;
                            break;
                        }
                    }
            ...
            } else if (strcmp(name, "MID") == 0 && event == TDL_BUTTON_PRESS_DOWN) {
                sg_app_ctx.state = STATE_BD_LIST;
                changed          = TRUE;
            }
        } else if (sg_app_ctx.state == STATE_BD_MSG) {
