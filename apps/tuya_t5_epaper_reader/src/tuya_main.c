...
static void draw_qrcode(int x0, int y0, int max_w, int max_h, const char *text)
{
    ...
    BOOL_T ok = qrcodegen_encodeText(text, temp, qrcode, qrcodegen_Ecc_LOW, qrcodegen_VERSION_MIN, qrcodegen_VERSION_MAX,
    ...
    if (sg_app_ctx.state == STATE_FILE_LIST) {
        ...
        if (sg_app_ctx.view_kind == VIEW_IMAGE) {
            ...
            if (Paint.Width < 600) {
                ...
                snprintf(status, sizeof(status), "UP/DN line  LT/RT page  SET rot  RST back  RST(hold)BD  %d%%", percent);
            }
        ...
        if (qr_w > 240) qr_w = 240;
        if (qr_h > 240) qr_h = 240;
        ...
        if (total_pages < 1) total_pages = 1;
        if (sg_bd_page < 0) sg_bd_page = 0;
        if (sg_bd_page > total_pages - 1) sg_bd_page = total_pages - 1;

        int page_start = sg_bd_page * items;
        int page_count = total - page_start;
        if (page_count < 0) page_count = 0;
        if (page_count > items) page_count = items;
        if (page_count == 0) sg_bd_selected = 0;
        if (sg_bd_selected < 0) sg_bd_selected = 0;
        if (sg_bd_selected > page_count - 1) sg_bd_selected = page_count - 1;
        ...
        for (int i = 0; i < page_count; i++) {
            ...
            char size_str[16];
            if (fi.is_dir) snprintf(size_str, sizeof(size_str), "DIR");
            else format_size_human(size_str, sizeof(size_str), fi.size);
            int size_px = (int)strlen(size_str) * (int)Font24.Width;
            int size_x = (int)Paint.Width - 10 - size_px;
            if (size_x < 10) size_x = 10;
        ...
        if (page_count == 0) {
            char msg[96];
            bdndk_message_get(msg, sizeof(msg));
            if (msg[0]) Paint_DrawString_EN(10, 50, msg, &Font24, BLACK, WHITE);
            else Paint_DrawString_EN(10, 50, "No files", &Font24, BLACK, WHITE);
        }

        Paint_DrawString_EN(10, Paint.Height - 28, "MID:Detail MID(hold):DL RST:Back RST(hold):SD", &Font24, BLACK, WHITE);
        ...
        char msg[96];
        bdndk_message_get(msg, sizeof(msg));
        if (!msg[0]) strncpy(msg, "Working...", sizeof(msg) - 1);
        msg[sizeof(msg) - 1] = 0;
...
static void handle_button_press(const char *name, TDL_BUTTON_TOUCH_EVENT_E event)
{
    ...
    } else if (sg_app_ctx.state == STATE_BD_AUTH || sg_app_ctx.state == STATE_BD_LIST || sg_app_ctx.state == STATE_BD_DETAIL ||
...
static void __tuya_main_task(void *param)
{
    ...
    while (1) {
        ...
        if (sg_app_ctx.state == STATE_BD_AUTH || sg_app_ctx.state == STATE_BD_LIST || sg_app_ctx.state == STATE_BD_DETAIL ||
            sg_app_ctx.state == STATE_BD_MSG) {
            BDNDK_VIEW_E v = bdndk_view_get();
            APP_STATE_E ns = sg_app_ctx.state;
            if (v == BDNDK_VIEW_AUTH) ns = STATE_BD_AUTH;
            else if (v == BDNDK_VIEW_LIST) ns = STATE_BD_LIST;
            else if (v == BDNDK_VIEW_DETAIL) ns = STATE_BD_DETAIL;
            else if (v == BDNDK_VIEW_MSG) ns = STATE_BD_MSG;
