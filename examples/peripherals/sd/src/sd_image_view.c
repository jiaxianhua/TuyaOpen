...
static void mem_print(const char *tag)
{
    ...
    if (psram_free >= 0) PR_NOTICE("%s heap_free=0x%x psram_free=0x%x", tag, heap_free, psram_free);
    else PR_NOTICE("%s heap_free=0x%x", tag, heap_free);
}
...
static void fit_aspect(int src_w, int src_h, int dst_w, int dst_h, int *out_w, int *out_h, int *out_off_x, int *out_off_y)
{
...
static int draw_bmp_any(const char *path, int x, int y, int w, int h)
{
    ...
    tkl_fclose(f);
    uint16_t bpp = le16(&hdr[28]);
    if (bpp == 24) return draw_bmp_24bit(path, x, y, w, h);
    if (bpp == 1) return draw_bmp_1bit(path, x, y, w, h);
    return -1;
}
...
static size_t tjpgd_infunc(JDEC *jd, uint8_t *buf, size_t len)
{
    ...
    if (!dev || !dev->file) return 0;
    if (!buf) {
        if (tkl_fseek(dev->file, (INT64_T)len, SEEK_CUR) != 0) return 0;
        return len;
    }
...
static int tjpgd_outfunc(JDEC *jd, void *bitmap, JRECT *rect)
{
    ...
    for (int ry = 0; ry < rect_h; ry++) {
        ...
        for (int rx = 0; rx < rect_w; rx++) {
            ...
            if (dx < dev->x || dy < dev->y || dx >= dev->x + dev->w || dy >= dev->y + dev->h) continue;
